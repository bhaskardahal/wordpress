...
// event start
	...
	function cs_event_meta_data($post) {
		...
		if ( $cs_event_meta <> "" ) {
			$cs_xmlObject = new SimpleXMLElement($cs_event_meta);
				...
				$event_gallery = $cs_xmlObject->event_gallery;
    	...
    	<div class="page-wrap">
            <div class="option-sec" style="margin-bottom:0;">
                <div class="opt-conts">
                   ...
                   <ul class="form-elements" id="event_ticket_options_id"  style="display:<?php if($event_ticket_options =="Buy Now" || $event_ticket_options =="")echo "inline"; else echo "none"; ?>" >
                        <li class="to-label">
                            <label>Buy now URL </label>
                        </li>
                        <li class="to-field"><input type="text" name="event_buy_now" value="<?php echo $event_buy_now?>" /></li>
                        <li class="full">&nbsp;</li>
                         <li class="to-label">
                            <label>Ticket Price</label>
                        </li>
						<li class="to-field"><input type="text" name="event_ticket_price" value="<?php echo $event_ticket_price?>" /></li>
                   </ul>
                   ...
                   <ul class="form-elements">
                        	...
                        	<select name="event_gallery" class="dropdown">
                                ...
                                <?php
									query_posts( array('posts_per_page' => "-1", 'post_status' => 'publish', 'post_type' => 'cs_gallery') );
										...
										?>
	                                        <option <?php if($event_gallery==get_the_ID())echo "selected"?> value="<?php the_ID()?>"><?php the_title()?></option>
                                        <?php
										endwhile;
                                ?>
                            </select>
                    ...
                    <ul class="form-elements">
						...
						<li class="to-field"><input type="text" name="event_phone_no" value="<?php echo $event_phone_no?>" /></li>
                   </ul>
                    <ul class="form-elements">
                        ...
                        <li class="to-field">
                        	...
                        	<select name="event_address" class="dropdown" onchange="javascript:inside_event_map_showhide(this)">
                            	<option value="0">--Select Location / Address--</option>
                                <?php
	...
	// event-location custom fields save start
		if ( isset($_POST['event_loc_meta_form']) and $_POST['event_loc_meta_form'] == 1 ) {
			...
			function event_loc_meta_save( $cs_post_id ) {
				...
				$sxe = new SimpleXMLElement("<event_loc></event_loc>");
					...
					$sxe->addChild('loc_country',$_POST['loc_country']);
						update_post_meta( $cs_post_id, 'cs_event_loc_meta', $sxe->asXML() );
			}
		}
	...
	// event custom fields save start
		if ( isset($_POST['event_meta_form']) and $_POST['event_meta_form'] == 1 ) {
			...
			function cs_event_meta_save( $cs_post_id ) {
				// repeating events start
				if ( isset($_POST['num_repeat'] ) ) {
					global $wpdb;
					$post_thumbnail_id = get_post_thumbnail_id( $cs_post_id );
					$post = get_post($cs_post_id);
					...
					$to_date = $_POST["event_to_date"];
						for ( $i = 1; $i < $_POST['num_repeat']; $i++ ) {
							...
							cs_events_meta_save($inserted_id);
							if ( $_POST['repeat'] <> 0 ) {
								...
								$cs_event_datetime=$_POST["event_from_date"].' '.$_POST["event_start_time"];
			  					update_post_meta( $cs_post_id, 'cs_event_from_date_time', $cs_event_datetime );
							}
						}
				}
				// repeating events end
				cs_events_meta_save($cs_post_id);
					update_post_meta( $cs_post_id, 'cs_event_from_date', $_POST["event_from_date"] );
					update_post_meta( $cs_post_id, 'cs_event_to_date', $_POST["event_to_date"] );
					$cs_event_datetime=$_POST["event_from_date"].' '.$_POST["event_start_time"];
					update_post_meta( $cs_post_id, 'cs_event_from_date_time', $cs_event_datetime );
			}
		}
